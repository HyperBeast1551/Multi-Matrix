...
// Enter the gyro caliberation offsets[if any (default 0)]
int16_t offsetX = 0;
int16_t offsetY = 0;
...
// Snake
int snakeX[64]; int snakeLen = 3; int foodX, foodY, snakeDir =1; unsigned long lastSnakeMove = 0; bool snakeGameOver = false;
// Falppy Dot
float birdY = 3.0; float birdVel = 0; float gravity = 0.04;
float pipeX = 7.0; int pipeGapY = 3; unsigned long lastFlapFrame = 0; bool flappyOver = false;
// 8-Ball & Dice
unsignedlong shakeTimer = 0; int randomResult = 0;
...
const uint8_t charE[8] = {0b01111110, 0b01100000, 0b01100000, 0b01111100, 0b01100000, 0b01100000, 0b01100000, 0};
const uint8_t charW[8] = {0b11000011, 0b11000011, 0b01100000, 0b11011011, 0b11111111, 0b11100111, 0b11000011, 0};
...
const uint8_t dice[6][8] = {
    ...
    {0,0, 0b01100000, 0b01100000, 0b00011000, 0b00011000, 0b00000110, 0b00000110, 0},
...
void resetSnake() {
    snakeLen=3; snakeX[0]=3; snakeY[0]=3; snakeX[1]=2; snakeY[1]=3; snakeX[2]=1; snakeY[2]=3; snakeDir=1; foodX=random(0,8); foodY=random(0,8); snakeGameOver=false;
}
...
void setMode(int newMode) {
    if (currentMode != newMode) {
        ...
        if (currentMode == 2) { ballRolling = flase;
        resetPins(); }
...
void setup() {
    ...
    mx.begin(); mx.control(MD_MAX72XX::INTENSITY, 2);
    mx.clear();
    Wire.begin(); Wire.setWireTimeout(3000, true);

    Wire.beginTransmission(Compass_addr); Wire.write(0x02);
    Wire.write(0x00); Wire.endTransmission();
    Wire.beginTransmission(MPU_addr); Wire.write(0x6B);
...
float getHeading() {
    int16_t x,y,z; Wire.beginTransmission(Compass_addr);
    Wire.write(0x03); Wire.endTransmission(false);
    Wire.requestFrom(Compass_addr, 6);
    if(Wire.available()>=6){ x=Wire.read()<<8|Wire.read();
    z=Wire.read()<<8|Wire.read(); y=Wire.read()<<8|Wire.read(); }
...
void runLetterCompass() {
    ...
    if (deg>=315 || deg<45) drawLetter(charN);
    else if (deg>=45 && deg<135) drawLetter(charE);
    else if (deg>=135 && deg<225) drawLetter(charS);
    else if (deg>=225 && deg<315) drawLetter(charW);
    delay(150);
}
...
void runSnakeGame() {
    if (snakeGameOver) { drawLetter(charN); delay(500);
    return; }
    int16_t calX, calY; readMPU(calX, calY);
    if (calX > 4000 && snakeDir != 3) snakeDir = 1;
    else if (calX < -4000 && snakeDir != 1) snakeDir = 3;
    else if (calY > 4000 && snakeDir != 0) snakeDir = 2;
    else if (calY < -4000 && snakeDir != 2) snakeDir = 0;
    ...
    if (millis() - lastSnakeMove > 350) {
        ...
        if(snakeX[0]<0 || snakeX[0]>7 || snakeY[0]<0 || snakeY[0]>7) {
            snakeGameOver=true; beep(150, 500;)
        }
...
void handleShake(int maxRand) {
    ...
    if (abs(calX) > 6000 || abs(calY) > 6000) {
        shakeTimer = millis(); randomResult = random(1, maxRand + 1);
        beep(500, 50);
    }
}
...
void runMagic8Ball() {
    ...
    if (millis() - shakeTimer < 1000) {
        if ((millis()/100)%2==0) drawLetter(charQ);
        else mx.clear();
    }
...
void runBubbleLevel() {
    int16_t calX, calY; readMPU(calX, calY);
    int ledX = map(calX, -4000, 4000, 0, 7);
    ledX = constrain(ledX, 0, 7);
    int ledY = map(calY, -4000, 4000, 0, 7);
    ledY = constrain(ledY, 0, 7);
    mx.clear();
    mx.setPoint(ledY, ledX, true);
    if (ledX == 3 && ledY == 3) beep(3000, 200);
    delay(30);
}